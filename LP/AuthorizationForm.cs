...
namespace WindowsFormsApp1
{
    public partial class AuthorizationForm : Form
    {
        ...
        private void buttonLogin_Click(object sender, EventArgs e)
        {
            ...
            command = new MySqlCommand("SELECT * FROM `customers` WHERE `login` = @uL AND `password` = @uP", db.GetConnection());
            command.Parameters.Add("@uL", MySqlDbType.VarChar).Value = loginUser;
            command.Parameters.Add("@uP", MySqlDbType.VarChar).Value = passwordUser;
        ...
        private void IdentificateUser(DataRow row)
        {
            ...
            {
                MainForm.masterRole = MainForm.Role.User;
                string userRole = "Пользователь";
                string userFullname = Convert.ToString(row["full_name"]);

                MainForm.userID = Convert.ToInt32(row["customer_id"]);
